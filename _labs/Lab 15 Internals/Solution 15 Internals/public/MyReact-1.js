...
function render(element, container, document) {
  ...
  const dom = element.type == "TEXT_ELEMENT" ? document.createTextNode("") : document.createElement(element.type); // assign properties to the node just created...
  ...
  element.props.children.forEach(child => render(child, dom, document)); // add the result to the root element... 
