...
function syncChildren(workItem, elements) {
  let index = 0; // grab the previous created workItem

  let oldWorkItem = workItem.alternate && workItem.alternate.child; // <-- newly added code

  let prevSibling = null;

  while (index < elements.length || oldWorkItem != null) {
    // <-- changed code
    const element = elements[index];
    let newWorkItem = null; // <-- changed code start

    const sameType = element && oldWorkItem && element.type == oldWorkItem.type; // compare the old workItem with the current element

    if (sameType) {
      // update the node in DOM
      newWorkItem = {
        type: oldWorkItem.type,
        props: element.props,
        dom: oldWorkItem.dom,
        parent: workItem,
        alternate: oldWorkItem,
        effectTag: "UPDATE"
      };
    }
    ...
    if (oldWorkItem) {
      oldWorkItem = oldWorkItem.sibling;
    } // <-- changed code end
...
function workLoop() {
  ...
  while (nextUnitOfWork) {
    //&& !shouldYield) {
    nextUnitOfWork = performWork(nextUnitOfWork); //shouldYield = deadline.timeRemaining() < 1
  }
...
function commitWork(workItem) {
  ...
  if (!workItem) return; // switch over the options here... add, change or removing items..
  ...
  const domParent = domParentTemp.dom; // switch over the options here... add, change or removing items..
...
function updateDom(dom, prevProps, nextProps) {
  ...
  Object.keys(nextProps).filter(isEvent).filter(isNew(prevProps, nextProps)).forEach(name => {
    const eventType = name.toLowerCase().substring(2); // console.log("adding even listener", eventType, name)
