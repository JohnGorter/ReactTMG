...
function performWork(workItem) {
  ...
  syncChildren(workItem, elements); // step 3. go one level deeper and set this workItem as the active one
...
function commitWork(workItem) {
  ...
  const domParent = workItem.parent.dom; // switch over the options here... add, change or removing items..
